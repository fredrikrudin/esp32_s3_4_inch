...
// Allokering av de globala LVGL-objekten
...
lv_obj_t *lbl_inverter_pwr = nullptr;
lv_obj_t *lbl_ruuvi_data = nullptr;
lv_obj_t *lbl_xiaomi_data = nullptr;

lv_obj_t *particle_mppt = nullptr;
lv_obj_t *particle_ip22 = nullptr;
lv_obj_t *particle_inverter = nullptr;

lv_anim_t anim_mppt;
lv_anim_t anim_ip22;
lv_anim_t anim_inverter;

// Callback-funktioner för linjeanimationer
...
void create_overview_page(lv_obj_t *parent) {
    ...
    // 1. STATISKA BAKGRUNDSLINJER (Victron Flödesbanor)
    static lv_point_t line_mppt_pts[] = { {55, 55}, {130, 140} };
    lv_obj_t *line_mppt = lv_line_create(parent);
    lv_line_set_points(line_mppt, line_mppt_pts, 2);
    lv_obj_set_style_line_color(line_mppt, lv_color_make(35, 35, 35), 0);
    lv_obj_set_style_line_width(line_mppt, 2, 0);

    static lv_point_t line_ip22_pts[] = { {425, 55}, {350, 140} };
    lv_obj_t *line_ip22 = lv_line_create(parent);
    lv_line_set_points(line_ip22, line_ip22_pts, 2);
    lv_obj_set_style_line_color(line_ip22, lv_color_make(35, 35, 35), 0);
    lv_obj_set_style_line_width(line_ip22, 2, 0);

    static lv_point_t line_inv_pts[] = { {240, 170}, {240, 240} };
    lv_obj_t *line_inv = lv_line_create(parent);
    lv_line_set_points(line_inv, line_inv_pts, 2);
    lv_obj_set_style_line_color(line_inv, lv_color_make(35, 35, 35), 0);
    lv_obj_set_style_line_width(line_inv, 2, 0);

    // 2. CENTRAL BATTERIMÄTARE (Shunt)
    ...
    // 3. EXTERNA GRÄNSSNITTSMÄTARE (MPPT & IP22)
    ...
    lbl_inverter_pwr = lv_label_create(parent);
    lv_obj_set_style_text_font(lbl_inverter_pwr, &lv_font_montserrat_14, 0);
    lv_obj_set_style_text_color(lbl_inverter_pwr, lv_color_make(41, 128, 185), 0);
    lv_obj_align(lbl_inverter_pwr, LV_ALIGN_BOTTOM_MID, 0, -55);
    ...
    // 4. ANIMATIONSPARTIKLAR
    particle_mppt = lv_obj_create(parent);
    lv_obj_set_size(particle_mppt, 6, 6);
    lv_obj_set_style_radius(particle_mppt, LV_RADIUS_CIRCLE, 0);
    lv_obj_set_style_bg_color(particle_mppt, lv_color_make(241, 196, 15), 0);
    lv_obj_set_style_border_width(particle_mppt, 0, 0);
    lv_obj_add_flag(particle_mppt, LV_OBJ_FLAG_HIDDEN);

    particle_ip22 = lv_obj_create(parent);
    lv_obj_set_size(particle_ip22, 6, 6);
    lv_obj_set_style_radius(particle_ip22, LV_RADIUS_CIRCLE, 0);
    lv_obj_set_style_bg_color(particle_ip22, lv_color_make(41, 128, 185), 0);
    lv_obj_set_style_border_width(particle_ip22, 0, 0);
    lv_obj_add_flag(particle_ip22, LV_OBJ_FLAG_HIDDEN);

    particle_inverter = lv_obj_create(parent);
    lv_obj_set_size(particle_inverter, 6, 6);
    lv_obj_set_style_radius(particle_inverter, LV_RADIUS_CIRCLE, 0);
    lv_obj_set_style_bg_color(particle_inverter, lv_color_make(231, 76, 60), 0);
    lv_obj_set_style_border_width(particle_inverter, 0, 0);
